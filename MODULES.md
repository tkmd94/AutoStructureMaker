...
| 解像度タイプ | 特徴 | 臨床用途 |
|:---|:---|:---|
| **Standard Resolution (STD)** | メモリ消費が少なく高速。Eclipse 上で標準的に作成される輪郭。 | 膀胱、直腸、肺、肝臓などの一般的な OAR や大まかな標的 |
| **High Resolution (HIGH)** | 微細なボクセル表現が可能。境界の凹凸や微小構造を忠実に再現。 | 視神経、視交叉、蝸牛、脊髄などの小体積 OAR や SRS / SBRT の GTV |
...
| バッジ表示 | 背景色 | 状態 | 説明 |
|:---|:---:|:---|:---|
| **`[HIGH]`** | ディープパープル (`#6A1B9A`) | 既存高解像度 | 現在の StructureSet に実在する高解像度輪郭 |
| **`[STD]`** | ブルーグレー (`#546E7A`) | 既存標準解像度 | 現在の StructureSet に実在する標準解像度輪郭 |
...
### 6.1 機能概要
指定した輪郭を、標準解像度から**高解像度セグメントへ昇格**させます。
