...
- ビルド成果物は `AutoStructureMaker\bin\x64\Release\AutoStructureMaker_v2.0.4.esapi.dll` に出力されます。
...
リポジトリ直下に配置された `test.bat` を実行することで、ソリューションのビルドと全 81 件の単体テストをワンクリックで連続実行できます。
