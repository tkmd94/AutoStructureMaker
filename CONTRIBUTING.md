...
- ビルド成果物は `AutoStructureMaker\bin\x64\Release\AutoStructureMaker_v2.0.3.esapi.dll` に出力されます。
...
リポジトリ直下に配置された `test.bat` を実行することで、ソリューションのビルドと全 60 件の単体テストをワンクリックで連続実行できます。
...
> **コミット前必須条件**: 全 60 件の単体テストが 100% PASS することを確認してください。
